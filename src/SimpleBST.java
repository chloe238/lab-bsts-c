...
public class SimpleBST<K,V> implements SimpleMap<K,V> {
  ...
  public V set(K key, V value) {
    ...
    while(temp != null){
      if(this.comparator.compare(temp.key, key) == 0){
        V tmp = temp.value;
        temp.value = value;
        return tmp;
      }
      else if(this.comparator.compare(temp.key, key) < 0){
        if(temp.right != null){
          temp = temp.right;
        }
        else{
  ...
  */
  ...
  Iterator<BSTNode<K,V>> nodes() {
    return new Iterator<BSTNode<K,V>>() {
      ...
      public BSTNode<K,V> next() {
        checkInit();
        stack.push(SimpleBST.this.root.right);
        return null;
      } // next();
  ...
  public void forEach(BiConsumer<? super K, ? super V> action, BSTNode<K, V> node) {
    if(node == null){
      return;
    } else {
